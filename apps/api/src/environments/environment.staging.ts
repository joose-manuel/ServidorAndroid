...
export const environment: Environment = {
  ...
  corsOrigins: (process.env['API_CORS_ORIGINS'] ?? 'https://staging-edge-admin.netlify.app').split(','),
  database: {
    host: process.env['DATABASE_HOST'] ?? 'localhost',
    port: Number(process.env['DATABASE_PORT'] ?? 5432),
    username: process.env['DATABASE_USER'] ?? 'edge',
    password: process.env['DATABASE_PASSWORD'] ?? '',
    database: process.env['DATABASE_NAME'] ?? 'edge_node_staging',
  },
  supabase: {
  ...
  turn: {
    serverUrl: process.env['TURN_SERVER_URL'] ?? 'turn:turn-staging.example.com:3478',
    serverDomain: process.env['TURN_SERVER_DOMAIN'] ?? 'turn-staging.example.com',
