...
@Component({
  ...
  template: `
    <hud-panel [title]="content.t('modem', 'title', 'Control de módem')">
      <div class="terminal">
        <div class="terminal__line">&gt; {{ content.t('modem', 'connect', 'modem.connect') }}(192.168.1.1)</div>
        <div class="terminal__line">&gt; {{ content.t('modem', 'status', 'status: ONLINE') }}</div>
      </div>
      <div class="actions">
        <cmd-button primary (cmdClick)="reboot()">{{ content.t('modem', 'rebootBtn', 'Reiniciar módem') }}</cmd-button>
      </div>
    </hud-panel>
  `,
  styles: [`
