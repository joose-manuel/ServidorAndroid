...
@Component({
  ...
  template: `
    <hud-panel title="Auditoría de red">
      <div class="terminal">
        <div class="terminal__line">&gt; edge-scan --network 192.168.1.0/24</div>
        <div class="terminal__line">&gt; El edge node escanea la red LAN cada 15 minutos</div>
        <div class="terminal__line">&gt; detecta dispositivos por ARP + ICMP ping</div>
      </div>
      <div class="actions">
        <cmd-button primary (cmdClick)="scan()">Escanear ahora</cmd-button>
      </div>
    </hud-panel>
  `,
  styles: [`
